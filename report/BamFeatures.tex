...
\begin{document}
    ...
    Since we are only looking for the most biologically plausible explanation, we only want to consider the most plausible case (\texttt{transcriptomic} $>$ \texttt{merged-transcriptomic} $gt$\texttt{intronic}). For example, if a read pair matches to a specific transcript of one gene but to the merged transcript of another gene, we would only consider the transcriptomic match, as it is more plausible. If a read pair matches to multiple transcript of multiple genes, we would want to know all the matched transcripts.
    \begin{figure}
        \centering
...
% Rest of output depends on what feature the read has
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{figures/BamFeatures/annot/nookaew_cm_log_rpkm_distribution}
        \caption
        {}

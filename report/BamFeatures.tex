...
\begin{document}
    ...
    Polymerase Chain Reaction (PCR) amplification is a crucial step in RNA sequencing workflows to increase the quantity of DNA fragments before sequencing. However, PCR amplification can introduce biases, such as the preferential amplification of certain fragments, leading to overrepresentation of some reads in the dataset. A possible reason for this is because the process is exponential, if a fragment is amplified, it increases the chance that it will be amplified again. This continues exponentially and can lead to the overrepresentation of certain fragments. These PCR duplicates can affect downstream analyses, including quantification of gene expression levels. The PCR index (\texttt{pcrindex}) is a metric used to identify read pairs that may have been amplified multiple times due to PCR cycles. It is calculated by tracking all observed read pairs and their associated regions (e.g., Region Vectors or \texttt{RV}s). If a read pair’s Region Vector matches previously observed regions, it is flagged as a potential PCR duplicate. This approach ensures that repeated fragments, whether arising from overamplification or technical artifacts, are systematically identified.
    ...
    To optimize computation time, the submission server will verify correctness using small test files. However, for evaluation, three large complete BAM files are provided. The objective of the project is to process the \texttt{ebna\_hisat.bam} file within one hour while using a maximum of 10GB of RAM. Additionally, the annotations of the read pairs in the test files will be analyzed. The other files used in this assignment are shown in Tables \ref{tab:test-bam-runtime} and \ref{tab:reference-bam-runtime}.
...
% TODO: Better caption
    ...
    \section{Supplement}
    The following sections provide additional details and usage examples.
